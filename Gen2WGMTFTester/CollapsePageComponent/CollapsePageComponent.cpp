#include "CollapsePageComponent.h"
    ...
    , d_ptr(new AdvancedToolBoxPrivate(this))
{
    ...
    //For the whole software, qApp sets the global
    QString stylesheet = "AdvancedToolBox::separator{width:1px; background: red;}";
    stylesheet += "AdvancedToolBox::tab{ padding: 10px; border: 0px solid;background: #ADADAD;color: white;}";
    stylesheet += "AdvancedToolBox::tab:hover{color: red;}";
    stylesheet += "AdvancedToolBox::branch:closed{image: url(:/Gen2WGMTFTester/images/routine/right_arrow.svg);width: 30px;padding: 5px;}";
    stylesheet += "AdvancedToolBox::branch{image: url(:/Gen2WGMTFTester/images/routine/right_arrow_down.svg);width: 30px;padding: 5px;}";
...
void AdvancedToolBox::SetLayout(QVBoxLayout* qVBoxLayout, int index, int stretch)
{
    qVBoxLayout->insertWidget(index, this, stretch);
}
...
void AdvancedToolBox::addWidget(QWidget* widget, const QString& label, const QIcon& icon)
{
    ...
    //Initialize drawer folding
    setItemExpand(0, false);
    QRect r = d->items[0]->tabContainer->geometry();
    this->setMinimumHeight(r.y());
}
...
void AdvancedToolBox::updateWidgetHeight(int height)
{
...
void AdvancedToolBox::mouseMoveEvent(QMouseEvent* event)
{
...
void AdvancedToolBoxPrivate::setIndexExpand(int index, bool expand)
{
    ...
    //
    if (expand)
        q->setMinimumHeight(q->m_currentHeight);
    else
        q->setMinimumHeight(items[0]->tabContainer->geometry().y());
    ...
    QObject::connect(title, &ToolBoxTitle::titleClicked, this, [this](int index)
        {
            ...
            if (item)
            {
                /*if (!item->isExpanded)q->setMinimumHeight(200);
                else q->setMinimumHeight(36);*/
...
CollpasePage::CollpasePage(QObject* parent) : QObject(parent)
{
    m_advancedToolBox = new AdvancedToolBox();
}

CollpasePage::~CollpasePage()
{
}

QWidget* CollpasePage::GetWidget()
{
    return m_advancedToolBox;
}

void CollpasePage::setItemExpand(int index, bool expand)
{
    m_advancedToolBox->setItemExpand(index, expand);
}

void CollpasePage::SetLayout(QVBoxLayout* qVBoxLayout, int index, int stretch)
{
    m_advancedToolBox->SetLayout(qVBoxLayout, index, stretch);
}

void CollpasePage::addWidget(QWidget* widget, const QString& label, const QIcon& icon)
{
    m_advancedToolBox->addWidget(widget, label, icon);
}


#include "CollapsePageComponent.moc"